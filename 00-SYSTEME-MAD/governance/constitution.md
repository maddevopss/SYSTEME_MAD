---
Projet: SYSTEME_MAD
Document: Constitution — Index des principes fondateurs
Version: 0.1
Dernière révision: 2026-07-15
Statut: Proposition
Auteur: Marc-André Dufour
Langue source: Français
---

# Constitution de SYSTEME_MAD

## Rôle

Cette Constitution est la porte d’entrée des principes fondateurs de SYSTEME_MAD.

Elle ne remplace pas les fondations, standards, ADR, guides ou registres. Elle indique où vivent les règles, précise leur rôle et permet aux bâtisseurs de comprendre l’ordre d’autorité du Canon.

SYSTEME_MAD n’est pas l’auteur de la connaissance. Il fournit le cadre qui permet aux bâtisseurs de la produire, de la valider, de la relier et de la préserver.

## Article I — Souveraineté des bâtisseurs

La connaissance, les décisions et leur responsabilité appartiennent aux bâtisseurs.

Référence fondatrice :

- [`01-FONDATIONS/souverainete-des-batisseurs.md`](../../01-FONDATIONS/souverainete-des-batisseurs.md)

Référence culturelle complémentaire :

- [`01-FONDATIONS/charte-des-batisseurs.md`](../../01-FONDATIONS/charte-des-batisseurs.md)

## Article II — Identité et non-négociables

L’identité de SYSTEME_MAD et ses limites fondamentales sont définies dans les fondations.

Références principales :

- [`01-FONDATIONS/00-premier-bloc.md`](../../01-FONDATIONS/00-premier-bloc.md)
- [`01-FONDATIONS/mad-001.md`](../../01-FONDATIONS/mad-001.md)
- [`01-FONDATIONS/non-negociable.md`](../../01-FONDATIONS/non-negociable.md)
- [`01-FONDATIONS/orientation-cognitive.md`](../../01-FONDATIONS/orientation-cognitive.md)
- [`01-FONDATIONS/infrastructure-intellectuelle-mad.md`](../../01-FONDATIONS/infrastructure-intellectuelle-mad.md)

## Article III — Construction de la connaissance

Les idées ne deviennent pas officielles par simple formulation. Elles suivent un cycle explicite de documentation, recherche, examen, validation et intégration.

Références principales :

- [`01-FONDATIONS/cycle-construction-connaissances.md`](../../01-FONDATIONS/cycle-construction-connaissances.md)
- [`01-FONDATIONS/convention-une-brique-une-pr.md`](../../01-FONDATIONS/convention-une-brique-une-pr.md)
- [`13-RESSOURCES/research/03-madproof/000-cadre-madproof.md`](../../13-RESSOURCES/research/03-madproof/000-cadre-madproof.md)

## Article IV — Sources d’autorité

L’autorité documentaire suit l’ordre général suivant :

1. le Manifeste et les règles de gouvernance applicables;
2. les fondations et non-négociables;
3. les standards;
4. les ADR;
5. les guides et playbooks;
6. les connaissances de référence;
7. les roadmaps et explorations non stabilisées.

Le statut YAML de chaque document demeure déterminant. Un document `Proposition`, `Brouillon` ou `À valider` ne possède pas la même autorité qu’un document `Officiel`.

Références principales :

- [`MANIFEST.md`](../../MANIFEST.md)
- [`00-SYSTEME-MAD/registry/REG-001-mad-registry.md`](../registry/REG-001-mad-registry.md)
- [`00-SYSTEME-MAD/yaml-standard.md`](../yaml-standard.md)

## Article V — Décisions et évolution

Une décision structurante doit être explicite, justifiée et traçable.

Les ADR documentent les choix qui modifient l’architecture, les pratiques, les standards ou la gouvernance. Les roadmaps orientent les travaux futurs sans se substituer aux décisions officielles.

Références principales :

- [`04-ADR/`](../../04-ADR/)
- [`10-ROADMAP/`](../../10-ROADMAP/)

## Article VI — Qualité et protection du Canon

La qualité repose sur des règles vérifiables, des checklists, des preuves et des mécanismes de contrôle.

Références principales :

- [`00-SYSTEME-MAD/governance/mad-guardian.md`](mad-guardian.md)
- [`03-STANDARDS/std-800-mad-guardian-quality-gates.md`](../../03-STANDARDS/std-800-mad-guardian-quality-gates.md)
- [`09-CHECKLISTS/chk-002-validation-documentaire.md`](../../09-CHECKLISTS/chk-002-validation-documentaire.md)
- [`05-PLAY/play-010-passe-qualite-documentaire.md`](../../05-PLAY/play-010-passe-qualite-documentaire.md)

## Article VII — Place des assistants

Les assistants peuvent contribuer à la recherche, à l’analyse, à la structuration, à la rédaction et à la vérification.

Ils proposent et signalent. Ils ne déterminent pas seuls ce qui devient officiel et ne peuvent recevoir la responsabilité d’une décision.

Toute contribution d’un assistant demeure soumise aux règles de gouvernance et à la validation des bâtisseurs.

## Parcours de lecture constitutionnel

Pour comprendre rapidement SYSTEME_MAD :

1. lire cette Constitution;
2. lire la souveraineté des bâtisseurs;
3. lire la Charte des Bâtisseurs;
4. lire les non-négociables;
5. lire le cycle de construction des connaissances;
6. consulter le Manifeste pour les règles documentaires;
7. suivre ensuite les standards et ADR applicables au chantier.

## Clause d’interprétation

Cette Constitution cartographie les autorités existantes. En cas de conflit, le document officiel le plus spécifique et la décision la plus récente explicitement applicable prévalent, sous réserve du Manifeste et des non-négociables.
