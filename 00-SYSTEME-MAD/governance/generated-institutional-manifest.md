---
Projet: SYSTEME_MAD
Document: Manifeste institutionnel généré — P5.1
Version: 1.0.0
Dernière révision: 2026-07-19
Statut: Officiel
Auteur: Marc-André Dufour
---

# Manifeste institutionnel SYSTEME_MAD — P5.1

- Version institutionnelle : **1.0.0**
- Statut : **Officiel**
- Commit de référence : `caf41671005771ca20ff72549a6f42b7aab10997`
- Documents déclarés : **8**
- Documents valides : **8**
- Documents invalides : **0**
- Manifeste valide : **oui**

| Document | Version | Statut | Validation | Empreinte Git |
|---|---|---|---|---|
| `MANIFEST.md` | 1.3 | Officiel | Valide | `192a0caec193eda3326b7b0801c62796d98534b1` |
| `00-SYSTEME-MAD/governance/constitution.md` | 1.1 | Officiel | Valide | `d1545024479d307efda6331154b109f2122d12c7` |
| `00-SYSTEME-MAD/yaml-standard.md` | 1.0 | Officiel | Valide | `5451652e790e583f052180de8d5495e24e4c0eda` |
| `00-SYSTEME-MAD/governance/contrat-stabilite-coeur-systeme-mad.md` | 1.0 | Officiel | Valide | `85b5c391b473819c9e5f8d9394098c8f90e5ad4f` |
| `09-CHECKLISTS/chk-010-stabilite-coeur-systeme-mad.md` | 1.0 | Officiel | Valide | `648a06f026c3a1ba9ab15d7ee817de916b14a81d` |
| `00-SYSTEME-MAD/governance/registre-preuves-ci-coeur-systeme-mad.md` | 1.0 | Officiel | Valide | `afe7bcee085eb0cbad594184c3c0dbf0df23d71f` |
| `00-SYSTEME-MAD/governance/perimetre-officiel-coeur-stable-v1.md` | 1.0 | Officiel | Valide | `42f6dd4b10063bf564b7285dbe309f12a662467c` |
| `00-SYSTEME-MAD/governance/declaration-stabilite-coeur-v1.md` | 1.0 | Officiel | Valide | `f8e62772a5836cd5454507c7f0082655c3e9a3b2` |

## Exclusions déclarées

- **Produits et implémentations** — MADSuite, les dépôts d’exécution et les implémentations techniques conservent leurs propres cycles de validation et de versionnement.
- **Domaines métier** — Les contenus métier ne sont pas inclus automatiquement dans le cœur institutionnel stable.
- **Recherches non stabilisées et continuité cognitive** — Les recherches, hypothèses et construits en développement demeurent soumis à MADPROOF et à une promotion explicite.
- **Fondations candidates** — Une fondation candidate n’entre dans le périmètre stable qu’après audit, décision et inclusion explicite.
- **Autres documents du dépôt** — Seuls les documents déclarés dans cette composition représentent la version institutionnelle du cœur stable v1.

> Ce manifeste décrit un sous-ensemble institutionnel explicitement déclaré. Il ne transforme pas automatiquement la branche main en publication officielle.
